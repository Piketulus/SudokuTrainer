    ...
    def test_create_random_solution_is_valid(self):
        ...
        for i in range(self.sudoku.size):
            for j in range(self.sudoku.size):
                ...
                self.assertTrue(self.gameService._valid_placement(self.sudoku.size, self.sudoku.solution, i, j, num))
        ...
        for i in range(self.sudoku.size):
            for j in range(self.sudoku.size):
                if self.sudoku.grid[i][j] != 0:
                    self.assertEqual(self.sudoku.solution[i][j], self.sudoku.grid[i][j])
